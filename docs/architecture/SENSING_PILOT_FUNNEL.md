...
The standalone public app includes only the sensing public_router, the Obligation Register intake routes (`/api/v1/sensing/obligation-register/intake` and `/privacy`, added 2026-09-10 for the one sellable offer), plus health/root routes. Sensing lead-admin routes such as lead listing, funnel updates, privacy export/delete, and conversion to BFMS projects are excluded from the public route map rather than relying only on environment-dependent auth flags. This route-boundary check is covered by tests/unit/test_public_sensing_deployment_boundary.py and tests/unit/test_sensing_pilot_funnel_contract.py.

Ruled 2026-09-10 (Stephen, decision D2 = A in MUNI-PAL lab/twin-bfms/governance/decision-request.md): production switches from munipal.main to munipal.sensing_app via railway.toml. Consequence, stated on purpose: the public site's `/auth` login and the `/pricing` Stripe checkout call `/api/v1/auth/*` and `/api/v1/stripe/*`, which the public app does not mount (Stripe checkout also requires an authenticated user, see src/munipal/api/routes/stripe.py). Those two paths were never the sales path (the offer is sold by letter and ACH; STAGE-DIAGNOSIS forbids pricing machinery at Stage 2) and are dead on the public site after the flip until the frontend removes or re-homes them.
