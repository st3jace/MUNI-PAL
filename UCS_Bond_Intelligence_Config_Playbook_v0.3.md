...
**Version:** 0.3 | **Created:** 2026-01-27 | **Updated:** 2026-02-01  
...
## 9. INFORMATION REQUEST TEMPLATES (WP8)

### 9.1 Purpose & Design Philosophy

When disclosure synthesis (WP7) generates TBD markers for missing facts, or checklist items remain blocked due to evidence gaps, the Information Request system provides structured guidance for filling those gaps. Each template is:

- **Bond-domain contextualized**: Explains WHY the information matters for bond issuance
- **Actionable**: Specifies WHO needs it, WHEN, and WHAT format is acceptable
- **Consequence-aware**: Describes impact on readiness score, checklist progress, and disclosure completeness
- **Example-driven**: Shows acceptable vs. unacceptable evidence with rationale

### 9.2 Information Request Template Catalog

The following templates are triggered when corresponding schema paths have TBD markers in disclosure documents or blocking checklist items:

| Template ID | Title | Trigger Fact Paths | Trigger Phase | Priority |
|---|---|---|---|---|
| `issuer_authority_001` | Inducement Resolution and Tax Status Confirmation | `governance.inducement`, `regulatory.tax-status` | P1 | Critical |
| `feedstock_supply_002` | Feedstock Supply Documentation and Confidence Assessment | `feedstock.supply.mechanism`, `feedstock.supply.confidence` | P2 | High |
| `offtake_revenue_003` | Commodity Offtake Arrangements and Revenue Validation | `revenue.offtake.status`, `revenue.commodities.list` | P3 | High |
| `cab_structure_004` | CAB Terms and Structure Documentation | `cab.originalprincipial`, `cab.accretionrate`, `cab.accretion.period.years` | P5 | Critical |
| `slb_verification_005` | SLB KPI Verification Methodology and Third-Party Verifier | `slb.kpi.1.verification.method` | P5 | High |
| `financial_model_006` | Financial Model Outputs and DSCR Coverage | `finmodel.outputs.dscrbase`, `finmodel.outputs.dscrstress` | P5 | Critical |
| `technology_specification_007` | Technology Specification and Performance Documentation | `technology.type`, `technology.throughput.nameplate`, `technology.throughput.annual` | P2 | Critical |
| `security_collateral_008` | Security Package and Collateral Structure Documentation | `security.revenue.pledge`, `security.equipment.schedule`, `security.realproperty` | P4 | Critical |
| `permitting_status_009` | Permitting Status and Regulatory Compliance Documentation | `permitting.air-quality.status`, `permitting.solidwaste.status` | P2 | High |
| `operating_expenses_010` | Operating Expense Budget and EBITDA Projections | `opex.total.annual`, `ebitda` | P4 | High |
| `slb_kpi_selection_011` | SLB KPI Selection, Baselines, and Target Setting | `slb.kpis.shortlist`, `slb.kpi.1.name`, `slb.kpi.1.baseline.value` | P5 | Critical |
| `project_description_012` | Canonical Project Description and Overview | `project.canonicaldescription` | P1 | High |

### 9.3 Template Structure

Each Information Request template includes:

```yaml
template_id: unique identifier
title_template: human-readable title
trigger_fact_paths: [schema paths that trigger this request when missing]
trigger_phase: checklist phase where gap is material
why_it_matters: explanation of bond-domain significance
who_needs_it: [stakeholders requiring this information]
when_needed: timeline requirement
consequences: impact of not providing information
regulatory_reference: applicable regulations/standards
guidance_overview: summary of what's needed
specific_questions: [detailed questions to answer]
data_points_needed: [specific facts to collect]
suggested_approach: step-by-step guidance
common_pitfalls: [mistakes to avoid]
time_estimate: expected effort
examples: [acceptable evidence examples with rationale]
acceptable_sources: [document types that satisfy requirement]
minimum_confidence: required confidence threshold
expected_format: deliverable specification
default_priority: critical|high|medium|low
default_owner: suggested responsible party
```

### 9.4 Owner Assignment Rules

Information requests are auto-assigned to owners based on schema path prefixes:

| Schema Path Prefix | Default Owner |
|---|---|
...
### 9.5 Information Request Workflow

1. **Generation**: When disclosure synthesis produces TBDs or checklist items are blocked, system auto-generates information requests from matching templates
2. **Assignment**: Requests assigned to default owner based on schema path mapping
3. **Acknowledgment**: Owner acknowledges receipt and target resolution date
4. **Submission**: Owner uploads supporting documents or provides fact values
5. **Resolution**: System extracts facts from submitted documents; if confidence meets threshold, request closes and TBD is resolved
6. **Escalation**: Requests not resolved within target date escalate to project sponsor

### 9.6 Disclosure-Information Request Integration

The Information Request system ensures continuity between disclosure TBDs and actionable gap-filling:

| Disclosure TBD | Generated Information Request | Resolution Path |
|---|---|---|
| `[TBD: Issuer not specified]` | `project_description_012` + `issuer_authority_001` | Upload IDA resolution, draft project summary |
| `[TBD: Technology type not specified]` | `technology_specification_007` | Upload OEM spec sheet, IE feasibility section |
| `[TBD: Security structure to be determined]` | `security_collateral_008` | Document revenue pledge, equipment schedule |
| `[TBD: KPIs pending selection]` | `slb_kpi_selection_011` | Draft SLB framework with KPI definitions |
| `[TBD: Verification method to be established]` | `slb_verification_005` | Engage verifier, document methodology |
| `[TBD: Air permit status not yet confirmed]` | `permitting_status_009` | Upload permit application status report |
| `[TBD: Base DSCR not yet calculated]` | `financial_model_006` | Upload financial model DSCR schedule |

---

## 10. BOND ISSUANCE PROCESS GOVERNANCE (EMPIRICALLY-INFORMED)
...
## 11. SAMPLE INFERENCE & CONFLICT RESOLUTION RULES
...
## 12. IMPLEMENTATION ROADMAP (FOR BFMS ENGINEERING)
...
| 0.3 | 2026-02-01 | [BFMS Team] | Added Section 9: Information Request Templates (WP8) - 12 templates covering all disclosure TBD triggers; ensures continuity between disclosure gaps, checklist items, and actionable information requests |
...
## 15. APPENDIX: CRITICAL SCHEMA PATH REFERENCE
