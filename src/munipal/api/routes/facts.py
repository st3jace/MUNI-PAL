...
from munipal.core.schemas.base import CriticalityTier, ReviewStatus
...
async def list_facts(
    ...
    schema_path_prefix: str | None = Query(
        None,
        description="Legacy alias for schema_path",
    ),
    min_confidence: float | None = Query(None, ge=0.0, le=1.0),
    criticality: CriticalityTier | None = None,
    include_archived: bool = Query(False, description="Include archived facts"),
    ...
    resolved_schema_prefix = schema_path_prefix or schema_path

    facts, total = await service.list_facts(
        project_id=project_id,
        status_filter=resolved_status,
        schema_path_prefix=resolved_schema_prefix,
...
@router.get("/status-counts", response_model=dict[str, int], include_in_schema=False)
...
async def get_fact(
    ...
    """
    authz = AuthorizationService(db)
    await authz.require_fact_read(user_id, fact_id)
...
async def review_fact(
    ...
    reviewer_id: UUID | None = Query(None, description="ID of user submitting review"),
    service: FactService = Depends(get_fact_service),
    _: str = Depends(require_roles("admin", "analyst")),
) -> ExtractedFactRead:
    """
    ...
    """
    authz = AuthorizationService(db)
    await authz.require_fact_write(user_id, fact_id)

    try:
        resolved_reviewer_id = reviewer_id or UUID(user_id)
        fact = await service.review_fact(fact_id, resolved_reviewer_id, review)
        return service.fact_to_read_schema(fact)
...
async def approve_fact(
    ...
    user_id: AuthenticatedUserId,
    fact_id: UUID,
    reviewer_id: UUID | None = Query(None, description="ID of user approving"),
    ...
    try:
        ...
        AuditService.emit_event(
            ...
            metadata={"reviewer_id": str(resolved_reviewer_id)},
        )
        return service.fact_to_read_schema(fact)
    except ValueError as e:
        raise HTTPException(
...
async def reject_fact(
    ...
    user_id: AuthenticatedUserId,
    fact_id: UUID,
    reviewer_id: UUID | None = Query(None, description="ID of user rejecting"),
...
async def request_revision(
    ...
    reviewer_id: UUID | None = Query(None, description="ID of user requesting revision"),
    service: FactService = Depends(get_fact_service),
    _: str = Depends(require_roles("admin", "analyst")),
) -> ExtractedFactRead:
    """
...
@router.post("/{fact_id}/archive", response_model=ExtractedFactRead)
async def archive_fact(
    db: DbSession,
    user_id: AuthenticatedUserId,
    fact_id: UUID,
    request: FactArchiveRequest,
    reviewer_id: UUID | None = Query(None, description="ID of user archiving"),
    service: FactService = Depends(get_fact_service),
    _: str = Depends(require_roles("admin", "analyst")),
) -> ExtractedFactRead:
    """
    ...
    try:
        resolved_reviewer_id = reviewer_id or UUID(user_id)
        fact = await service.archive_fact(
            fact_id=fact_id,
            reviewer_id=resolved_reviewer_id,
        ...
        AuditService.emit_event(
            ...
            project_id=str(fact.project_id),
            metadata={
                "reviewer_id": str(resolved_reviewer_id),
                "reason_code": request.reason_code,
            },
        )
        return service.fact_to_read_schema(fact)
...
@router.post("/{fact_id}/unarchive", response_model=ExtractedFactRead)
async def unarchive_fact(
    db: DbSession,
    user_id: AuthenticatedUserId,
    fact_id: UUID,
    request: FactUnarchiveRequest,
    reviewer_id: UUID | None = Query(None, description="ID of user unarchiving"),
    service: FactService = Depends(get_fact_service),
    _: str = Depends(require_roles("admin", "analyst")),
) -> ExtractedFactRead:
    """
    ...
    try:
        resolved_reviewer_id = reviewer_id or UUID(user_id)
        fact = await service.unarchive_fact(
            fact_id=fact_id,
            reviewer_id=resolved_reviewer_id,
            note=request.note,
        )
...
@router.get("/conflicts/", response_model=dict[str, list[dict]])
async def find_conflicts(
    ...
    authz = AuthorizationService(db)
    await authz.require_project_read(user_id, project_id)
    conflicts = await detector.find_conflicts(project_id, schema_path)
    return {"conflicts": conflicts}


@router.get("/conflicts/queue", response_model=dict[str, list[dict]])
async def get_conflict_queue(
    db: DbSession,
    user_id: AuthenticatedUserId,
    project_id: UUID,
    criticality: str | None = Query(None, description="Filter by criticality"),
    phase: str | None = Query(None, description="Filter by phase (P1-P6)"),
    max_age_days: int | None = Query(None, ge=0, description="Only include older pending items"),
    service: FactService = Depends(get_fact_service),
    _: str = Depends(require_roles("admin", "analyst", "viewer")),
) -> dict[str, list[dict]]:
    """
    ...
    queue = await service.get_conflict_queue(
        project_id=project_id,
        criticality=criticality,
        phase=phase,
...
@router.post("/conflicts/resolve", response_model=dict[str, list[dict]])
async def resolve_conflicts(
    ...
    try:
        resolutions = await detector.auto_resolve_conflicts(project_id, strategy)
