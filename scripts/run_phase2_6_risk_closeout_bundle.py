"""
Run Phase 2-6 risk closeout bundle and write evidence artifacts.

This bundle targets active non-Phase-10 risks:
...
def _write_markdown_report(
    *,
    ...
    lines: list[str] = [
        "# Phase 2-6 Risk Closeout Bundle Report",
        "",
    ...
    lines.extend(
        [
            ...
            "- [ ] Capture latest CI run URL for `.github/workflows/phase2-6-risk-closeout-dispatch.yml`.",
            "- [ ] Mark `R-001`, `R-002`, and `R-003` status in `V2/EXECUTION_TRACKER.md` based on this bundle + CI evidence.",
            "- [ ] Update Phase 2-6 status row ownership and completion notes in `V2/EXECUTION_TRACKER.md`.",
            "",
        ]
    )
...
def main() -> int:
    ...
    stamp = _timestamp()
    report_root = repo_root / "reports" / "phase2_6_closeout"
    ...
    commands: list[tuple[str, str, Mapping[str, str] | None]] = [
        (
            "OpenAPI Contract Snapshot Gate",
            "pytest -q tests/contract/test_openapi_contract.py -p no:cacheprovider",
            default_env,
        ),
        (
            "OpenAPI Generated Artifact Gate",
            "python scripts/verify_openapi_generated_artifacts.py --fail-on-mismatch",
            default_env,
        ),
        (
            "Core Flow Regression Slice",
            "pytest -q "
        ...
        (
            "Security Tenant Risk Regression Slice",
            "pytest -q "
        ...
        (
            ...
            "python scripts/assess_test_suite_health.py --enforce --min-expected-tests 120",
            default_env,
        ),
        (
            "Frontend Tests",
    ...
    for name, command, extra_env in commands:
        print(f"[phase2-6-closeout] running: {name}")
        results.append(
            _run_command(
                name=name,
                command=command,
    ...
    json_report_path = report_root / f"phase2_6_closeout_{stamp}.json"
    md_report_path = report_root / f"phase2_6_closeout_{stamp}.md"
    ...
    print(f"[phase2-6-closeout] overall={overall_status}")
    print(f"[phase2-6-closeout] markdown={md_report_path.relative_to(repo_root).as_posix()}")
    print(f"[phase2-6-closeout] json={json_report_path.relative_to(repo_root).as_posix()}")
