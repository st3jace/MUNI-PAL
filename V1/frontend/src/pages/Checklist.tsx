...
const criticalityConfig: Record<CriticalityTier, { label: string; color: string; bg: string }> = {
  [CriticalityTier.CRITICAL]: {
    label: 'Critical',
    color: 'text-red-700',
    bg: 'bg-red-100',
  },
  [CriticalityTier.MATERIAL]: {
    label: 'Material',
...
export default function Checklist() {
  ...
  const groupedMissingPaths = missingPaths?.reduce(
    (acc, path) => {
    ...
    {} as Record<CriticalityTier, MissingPathInfo[]>
  ) || {}
      ...
      <div className="card p-6">
        ...
        <div className="flex items-center gap-2">
          {phases.map((phase, index) => {
            ...
            const canProceed = summary?.can_proceed_to_next ?? false
      ...
      <div className="card">
        ...
        {missingPathsLoading ? (
          ...
          <div className="divide-y divide-gray-200">
            ...
            {groupedMissingPaths[CriticalityTier.CRITICAL]?.length > 0 && (
              <div className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  <AlertCircle className="h-4 w-4 text-red-500" />
                  <span className="text-sm font-medium text-red-700">
                    Critical ({groupedMissingPaths[CriticalityTier.CRITICAL].length})
                  </span>
                </div>
            ...
            {/* Material items */}
            {groupedMissingPaths[CriticalityTier.MATERIAL]?.length > 0 && (
              <div className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  <Clock className="h-4 w-4 text-yellow-500" />
                  <span className="text-sm font-medium text-yellow-700">
                    Material ({groupedMissingPaths[CriticalityTier.MATERIAL].length})
                  </span>
                </div>
                <div className="space-y-2">
            ...
            {groupedMissingPaths[CriticalityTier.SECONDARY]?.length > 0 && (
              <div className="p-4">
                <details>
                  <summary className="flex items-center gap-2 cursor-pointer">
                    <Circle className="h-4 w-4 text-gray-400" />
                    <span className="text-sm font-medium text-gray-600">
                      Secondary ({groupedMissingPaths[CriticalityTier.SECONDARY].length})
                    </span>
                  </summary>
